...
export const dismissIdentityError = createAction(DISMISS_ERROR, id => ({ id, category: 'identity' }))
...
export function hydrateTokens() {
  ...
  if(!tokens)
    return setIdentityError(new IdentityError('Tokens did not exist in cookie.'))
...
const extractTokens = response => {
  ...
  if(typeof response === 'string' && response.includes('AllowOneTime')) {
    let loginLocation = window.location.port === '7000' ? '/' : '/Forms/Identity/Authorize.aspx'
    goToPath(`${loginLocation}?pwreset=${response.split('|')[1]}&case=true`)
  }
...
export function authorizeIdentity(credentials) {
  return (dispatch, getState) => {
    dispatch(fetchIdentity('authorize', credentials))
    let { identity } = getState()
    const { username, password, rememberMe } = credentials
    if(!(username && password))
      return dispatch(setIdentityError(new IdentityError('Requires username and password')))
    return dispatch(apiAction(['identity', 'authorize'],  { username: username
      ...
      .catch(err => dispatch(setIdentityError(new IdentityError('Unexpected error during authorize identity', err))))
  }
}
...
export function refreshIdentity(credentials = {}) {
  return (dispatch, getState) => {
    ...
    if(!refreshToken)
      return dispatch(setIdentityError(new IdentityError('No refresh token exists to refresh identity.')))
