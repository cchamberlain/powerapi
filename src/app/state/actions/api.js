...
import { apis } from 'config-client'
...
export const dismissError = createAction(DISMISS_ERROR, (id, category) => ({ id, category }))
...
const getTixClaimSchema = name => `http://schemas.powerapi.io/identity/claims/${name}`
...
const resolveBody = (actionConfig, inputData) => {
  ...
  let resolvedBody = matchedVariables.reduce((obj, key) => Object.assign({}, obj, { [key]: inputData[key].toString() }), {})
    ...
    return fetch(input, init)
      .then(response => {
        if(response.status >= 400) {
          throw new APIError('Bad response from server', null, { ...info, response })
        }
      ...
      .catch(err => {
        throw new APIError('Error during executeFetch.', err, info)
      })
  } catch(err) {
    return new APIError('Unexpected error occured during executeFetch.', err, { input, init, apiName, actionName, inputData })
  }
}
